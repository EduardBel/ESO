...
//------------------------------------------------------------------------------
	...
	_gg_escribir("*** Carga de programa SUPI.elf\n", 0, 0, 0);
	start = _gm_cargarPrograma("SUPI");
	if (start)
	{	
		_gp_crearProc(start, 1, "SUPI", 3);
		_gp_crearProc(start, 2, "SUPI", 3);
		_gp_crearProc(start, 3, "SUPI", 3);
